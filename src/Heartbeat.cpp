...
void Heartbeat::measure_heartbeat()
{
    ...
    float reads[heartbeat_samp_siz], sum;
    long int now, ptr;
    ...
    for (int i = 0; i < heartbeat_samp_siz; i++)
        reads[i] = 0;
    sum = 0;
    ptr = 0;
    ...
    while (task_at_start == Util::pot_val)
    {
        ...
        {
            rise_count++;
            if (!rising && rise_count > heartbeat_rise_threshold)
            {
